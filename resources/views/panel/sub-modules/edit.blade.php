...
<div class="card theme-card">
  <div class="card-header">
    <div class="d-flex justify-content-between align-items-start flex-wrap w-100">
      <div>
        <h6 class="mb-0">Edit Submodule</h6>
  ...
  <form method="POST" action="{{ route('panel.sub-modules.update', $subModule->id) }}" id="formId">
    @csrf 
    @method('PUT')
    <div class="card-body row g-3 mb-0">
      <div class="col-md-6">
        ...
        <select class="form-select form-select-sm select2" name="module_id" id="module_id">
          ...
          @foreach($moduleList as $module)
            <option value="{{ $module->id }}" {{ old('module_id', $subModule->module_id) == $module->id ? 'selected' : '' }}>
                {{ $module->name }}
            </option>
          @endforeach
        </select>
      ...
      <div class="col-md-6">
        ...
        <input type="text" class="form-control form-control-sm" name="name" id="name" value="{{ old('name', $subModule->name) }}" placeholder="Enter name" autocomplete="name" required autofocus>
      ...
      <div class="col-md-6">
        ...
        <select class="form-select form-select-sm select2" name="status" id="status">
          ...
          @foreach($statusList as $value => $label)
            <option value="{{ $value }}" {{ old('status', $module->status) == $value ? 'selected' : '' }}>
                {{ $label }}
            </option>
          @endforeach
        </select>
