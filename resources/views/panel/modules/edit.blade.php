...
<div class="card theme-card">
  ...
  <form method="POST" action="{{ route('panel.modules.update', $module->id) }}" id="formId">
    @csrf 
    @method('PUT')
    <div class="card-body row g-3 mb-0">
      <div class="col-md-6">
        ...
        <input type="text" class="form-control form-control-sm" name="name" id="name" value="{{ old('name', $module->name) }}" placeholder="Enter name" autocomplete="name" required>
        @error('name') <div class="text-danger small">{{ $message }}</div> @enderror
      </div>
      <div class="col-md-6">
        <label for="status" class="form-label">Status</label>
        <select class="form-select form-select-sm select2" name="status" id="status">
          <option value="" readonly {{ old('status', $module->status) === null ? 'selected' : '' }}>Select Status</option>
