...
@section('content')
  ...
  <div class="d-flex justify-content-between align-items-start flex-wrap">
    <div>
      <h6 class="mb-0">Manage Module</h6>
    ...
    <div class="d-flex gap-2 mt-md-1 mt-lg-1">
      <div class="dropdown">
        ...
        <form method="GET" action="{{ route('panel.modules.index') }}" class="dropdown-menu p-3" 
      ...
      <a href="{{ route('panel.modules.create') }}" class="btn btn-sm btn-outline-secondary px-3">
  ...
  <div class="table-responsive py-1">
        ...
        @if ($modules->count())
          @foreach ($modules as $module)
            <tr>
              ...
              <td class="text-center">{{ $module->created_at ? $module->created_at->diffForHumans() : '' }}</td>
              <td class="text-center">{{ \App\Enums\Status::tryFrom($module->status)?->label() ?? '' }}</td>
              <td class="text-nowrap">
                <div class="d-flex justify-content-center gap-3">
                  <a href="javascript:void(0)" class="text-primary" onclick="seeModuleDetail({{ $module->id }})">
                    <i class="fas fa-eye"></i>
                  </a>

                  <a href="{{ route('panel.modules.edit', $module->id) }}"
                     class="text-primary"
                     title="Edit">
                  ...
                  <form id="delete-{{ $module->id }}"
                        action="{{ route('panel.modules.destroy', $module->id) }}"
                        method="POST"
                        style="display: none;">
                      @csrf
                      @method('DELETE')
    ...
    <div class="d-flex justify-content-between align-items-center">
        ...
        {{ $modules->links('vendor.pagination.custom') }}
    </div>
  </div>
  </div>
...
@push('scripts')
  <script>
    function seeModuleDetail(id) {
      let url = "{{ route('panel.modules.show', ['module' => ':id']) }}".replace(':id', id);
      showModalContent(url, 'Module Details');
    }
  </script>
@endpush
