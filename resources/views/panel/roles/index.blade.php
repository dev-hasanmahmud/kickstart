...
@section('content')
  ...
  <div class="d-flex justify-content-between align-items-start flex-wrap">
    ...
    <div class="d-flex gap-2 mt-md-1 mt-lg-1">
      <div class="dropdown">
        ...
        <form method="GET" action="{{ route('panel.roles.index') }}" class="dropdown-menu p-3" 
          ...
          <div class="d-flex justify-content-end gap-2 mt-2">
            <a href="{{ route('panel.roles.index') }}" class="btn btn-sm btn-light">Reset</a>
            <button type="submit" class="btn btn-sm btn-secondary">Filter</button>
          </div>
        </form>
      </div>
      <a href="{{ route('panel.roles.create') }}" class="btn btn-sm btn-outline-secondary px-3">
  ...
  <div class="table-responsive py-1">
    <table id="rolesTable" class="custom-table mb-1">
        ...
        @if ($roles->count())
          @foreach ($roles as $role)
            <tr>
              ...
              <td class="text-center">{{ $role->created_at ? $role->created_at->diffForHumans() : '' }}</td>
              <td class="text-center">{{ \App\Enums\Status::tryFrom($role->status)?->label() ?? '' }}</td>
              <td class="text-nowrap">
                <div class="d-flex justify-content-center gap-3">
                  <a href="javascript:void(0)" class="text-primary" onclick="seeRoleDetail({{ $role->id }})">
                  ...
                  <form id="delete-{{ $role->id }}"
                        action="{{ route('panel.roles.destroy', $role->id) }}"
                        method="POST"
                        style="display: none;">
                      @csrf
                      @method('DELETE')
...
@push('scripts')
  <script>
    function seeRoleDetail(id) {
      let url = "{{ route('panel.roles.show', ['role' => ':id']) }}".replace(':id', id);
      showModalContent(url, 'Role Details');
    }
  </script>
@endpush
